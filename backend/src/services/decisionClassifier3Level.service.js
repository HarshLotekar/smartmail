...
/**
 * Calculate decision level and confidence
 */
function calculateDecisionLevel(email) {
  ...
  // Determine final level based on thresholds
  let finalLevel = 0;
  if (score >= 0.75) {
    finalLevel = 2;
  } else if (score >= 0.55) {
...
export async function classifyEmail(email, userId = null) {
  ...
  console.log(`[DECISION_CLASSIFIER] ${levelEmoji} Level ${result.level} - Score: ${result.confidence.toFixed(2)}`);
