-- SmartMail Database Schema (Fixed Order)
...
-- Email_Decisions Table
-- Stores AI decision classification metadata for emails
CREATE TABLE IF NOT EXISTS email_decisions (
    ...
    decision_type TEXT NOT NULL, -- 'reply_required', 'deadline', 'follow_up', 'informational_only'
    ...
    -- Ensure one decision per email per user
...
CREATE INDEX IF NOT EXISTS idx_email_decisions_detected_at ON email_decisions (detected_at DESC);

-- Composite indexes for common queries on email_decisions
...
CREATE INDEX IF NOT EXISTS idx_email_decisions_user_type ON email_decisions (user_id, decision_type);
CREATE INDEX IF NOT EXISTS idx_email_decisions_user_pending ON email_decisions (user_id, status, decision_required);
